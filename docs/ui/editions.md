...
- **Navigation**: both headers carry the **Photography | Editions switch** (`components/SpaceSwitch.tsx`, routing rules in `components/spaceSwitchRoutes.ts`). It sits right after the logo (Editions header from `sm`, photography header from `md`) and as a full-width row directly under the header on phones. The photography phone menu carries it too. Switching returns people to the last page they visited on the other side, maps `/account` ↔ `/editions/studio` once Web3 is on, and maps a published platform photo edition to its photo page; Choosing a side slides the pill across, then navigates with `navigate(to, { viewTransition: true })` so the whole page, header included, cross-fades (timing in `src/styles/index.css`; reduced motion skips both). Don't key `RootLayout`'s `<Outlet />` by side: routes have their own `Suspense`, and a remount fades into the "Loading..." fallback instead of the new page. The Editions header account button opens a menu (Your studio, Your public page when Web3 is on, Photography account, Sign out). The rail's "Create an edition" goes to `/editions/studio?section=create`, "Your studio" to `/editions/studio`, "Account settings" to `/account`. Old `/editions?mint=1` links redirect to the studio.
...
| `EditionsModal`                                            | `title`, `eyebrow?`, `onClose`, `footer?`, `size?: "md" \| "lg"`. Bottom sheet on mobile, centred dialog on desktop; locks body scroll and closes on Escape. Prefer it for new modals.                                                                                                                                  |
...
**Modal rules.** Every modal on this side — including the hand-rolled ones (`PresaleBuyModal`, `MintEditionModal`, `ConnectWalletModal`, `CryptoQrCodeModal`, `CertificateOfAuthenticityModal`) — uses `--ed-*` tokens, sets `role="dialog"` + `aria-modal` + an `aria-label`, closes on Escape, calls `useBodyScrollLock(isOpen)`, and takes its buttons from `primaryButtonClass` / `secondaryButtonClass` (they carry the right disabled and press states — don't hand-roll `disabled:opacity-50`). Copy is sentence case. The only hard-coded colours are coin brand marks (`SUPPORTED_COINS[].color`) and the white plate behind a QR code, which needs a light ground to scan. `ConvertBalanceModal`, `GiftNscModal` and `TransferCryptoModal` belong to the photography account vault and admin — leave those in photography styling.
...
- **Prices:** every edition is priced in GBP (`priceGbp`, the source of truth). `pricesFromGbp()` derives USD, ETH and SOL at fixed rates, and `gbpToNsc()` gives the NSC price at `NSC_PER_GBP` — 1 NSC = £1, the same rate checkout charges and the account uses to convert £ into NSC. Show collectors **NSC first, then GBP, then ETH**: `PRICE_CURRENCY_OPTIONS` for the currency switch, `formatPrice(price, currency)` for the main figure and `formatOtherPrices(price, currency)` for the "≈ £… · … ETH" line under it. Never compute NSC by hand. Studio screens stay in GBP because creators set prices in pounds. The presale sells NSC below £1 (`NscPresaleConfig.priceUsd`); that gap is the presale discount, and the banner states the £1 value.
